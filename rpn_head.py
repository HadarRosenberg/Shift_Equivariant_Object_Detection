...
from project_utils.activation import UpPolyActPerChannel
...
@HEADS.register_module()
class AFC_RPNHead(RPNTestMixin, AnchorHead):
    """RPN head.
    ...
    def __init__(self, in_channels, **kwargs):
        super(AFC_RPNHead, self).__init__(1, in_channels, **kwargs)

    def _init_layers(self):
        """Initialize layers of the head."""
        self.rpn_conv = nn.Conv2d(
            self.in_channels, self.feat_channels, 3, padding=1, padding_mode='circular')
        self.rpn_cls = nn.Conv2d(self.feat_channels,
                                 self.num_anchors * self.cls_out_channels, 1)
        self.rpn_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 4, 1)
        self.activate = UpPolyActPerChannel(channels = self.feat_channels,
                                            in_scale=1,
                                            out_scale=1,
                                            train_scale=True)
    ...
    def loss(self,
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        losses = super(AFC_RPNHead, self).loss(
            cls_scores,
            bbox_preds,
            gt_bboxes,
            None,
