...
class ConvNeXtAFC(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        # [hm] remove head (not used in backbone)
        ...
        # norm_layer = partial(LayerNorm, eps=1e-6, data_format="channels_first", normalization_type=normalization_type)
        for i_layer in range(4):
            # layer = norm_layer(dims[i_layer])
            layer = get_norm_layer(normalization_type, dims[i_layer], data_format='channels_first', **normalization_kwargs)
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            # print("[convnext_aal] load checkpoint from: ", pretrained)
            self.apply(_init_weights)
            logger = get_root_logger()
            
            # load_checkpoint(self, pretrained, strict=False, logger=logger)
            
            # [hm] TODO: maybe we have to change this, because this may be called with a different format 
            # (e.g. full MaskRCNN checkpoint instead of ConvNeXt checkpoint)
            logger.info(f'[convnext_afc] load model from: {pretrained}')
            sd = torch.load(pretrained, map_location='cpu')['model_ema']
            # load state_dict and print missing / unexpected keys
            missing, unexpected = self.load_state_dict(sd, strict=False)
            if missing:
                logger.warning(f'[convnext_afc] missing keys: {missing}')
            if unexpected:  
                logger.warning(f'[convnext_afc] unexpected keys: {unexpected}')
